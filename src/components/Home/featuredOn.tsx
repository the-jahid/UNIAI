...
class Media {
  ...
  addClickHandler() {
    // Add click event to the plane
    this.plane.program.gl.canvas.addEventListener("click", (e) => {
      // Calculate if the click is on this plane
      const rect = this.plane.program.gl.canvas.getBoundingClientRect()
      ...
      // Check if this plane is in the center (active)
      if (Math.abs(this.plane.position.x) < this.plane.scale.x / 2) {
        window.open("https://form.typeform.com/to/Mk0sQjuc", "_blank")
      }
    })
  }
...
class App {
  ...
  // New properties for dot navigation
  ...
  createMedias(
    ...
    this.originalItemsCount = galleryItems.length
  ...
  createDotsNavigation() {
    ...
    this.dotsContainer.className = "gallery-dots-container"
    this.container.appendChild(this.dotsContainer)

    // Create dots based on original items count (not duplicated)
  ...
  updateActiveDot(index: number) {
    // Normalize index to be within the range of original items
    const normalizedIndex = index % this.originalItemsCount
  ...
  onCheck() {
    ...
    // Update the active dot
    this.updateActiveDot(itemIndex)
  }
  ...
  update() {
    ...
    if (this.medias) {
      ...
      if (this.medias[0]) {
        const width = this.medias[0].width
        const currentIndex = Math.round(Math.abs(this.scroll.current) / width) % this.originalItemsCount
